...
function numericSort<U> (x: U[] | number[], sortFunc?: (a: U, b: U) => number): number[] {
  // @ts-expect-error: who cares
  if (sortFunc === undefined) sortFunc = (a, b) => a - b
  // @ts-expect-error: who cares
  return x.slice().sort(sortFunc)
}

function uniqueCountSorted (x: number[]): number {
 ...
 * ssq(0, 1, [-1, 0, 2], [1, 1, 5]);
 */
function ssq (j: number, i: number, sums: number[], sumsOfSquares: number[]): number {
  let sji // s(j, i)
  if (j > 0) {
 ...
 * = [[-1, -1, -1, -1], [2, 2, 2], [4, 5, 6]]);
 */
export default function ckmeans<U> (x: U[] | number[], nClusters: number, sortFunc?: (a: U, b: U) => number): U[][] | number[][] {
  ...
  const sorted = numericSort(x, sortFunc)
