...
const SUB_TEST_SHADER_BUFFER_LAYOUT_ATTR: Iterable<GPUVertexAttribute> = [0, 1, 2, 3, 4].map(
  (i) => ({
    shaderLocation: i,
...
export class GlyphFeature implements GlyphFeatureSpec {
  ...
  duplicate(tile: Tile, parent?: Tile, bounds?: BBox): GlyphFeature {
    ...
    const newGlyphAttributeNoStrokeBuffer = context.duplicateGPUBuffer(
      glyphAttributeNoStrokeBuffer,
      cE,
    );
    ...
    return new GlyphFeature(
      ...
      isIcon,
      featureCode,
      newGlyphUniformBuffer,
      newGlyphBoundsBuffer,
      newGlyphAttributeBuffer,
      newGlyphAttributeNoStrokeBuffer,
  ...
  #buildFilterBindGroup(): GPUBindGroup {
    ...
    const { context, glyphFilterBindGroupLayout, glyphBBoxesBuffer, glyphFilterResultBuffer } =
      workflow;
  ...
  #buildInteractiveBindGroup(): GPUBindGroup {
    ...
    const { context, glyphInteractiveBindGroupLayout, glyphBBoxesBuffer, glyphFilterResultBuffer } =
      workflow;
    return context.device.createBindGroup({
...
export default class GlyphWorkflow implements GlyphWorkflowSpec {
  ...
  async setup(): Promise<void> {
    ...
    this.glyphBBoxesBuffer = await context.buildGPUBuffer(
      'Glyph BBoxes Buffer',
      new Float32Array(Array(2_000 * 5).fill(0)),
      GPUBufferUsage.STORAGE,
    );
    this.glyphFilterResultBuffer = context.buildGPUBuffer(
      'Glyph Filter Result Buffer',
      new Float32Array(Array(2_000).fill(0)),
  ...
  buildSource(glyphData: GlyphData, tile: Tile): void {
    ...
    const {
      glyphFilterBuffer,
      glyphQuadBuffer,
      glyphQuadIDBuffer: glyphQuadIndexBuffer,
      glyphColorBuffer,
      featureGuideBuffer,
    } = glyphData;
    // prep buffers
    ...
    const source: GlyphSource = {
      type: 'glyph' as const,
      glyphFilterBuffer: context.buildGPUBuffer(
        'Glyph Filter Buffer',
        glyphFilterBuffer,
        GPUBufferUsage.STORAGE | GPUBufferUsage.VERTEX,
      ),
      glyphQuadBuffer: context.buildGPUBuffer(
        'Glyph Quad Buffer',
        glyphQuadBuffer,
        GPUBufferUsage.VERTEX,
      ),
      glyphQuadIndexBuffer: context.buildGPUBuffer(
        'Glyph Quad ID Buffer',
        glyphQuadIndexBuffer,
        GPUBufferUsage.VERTEX,
      ),
      glyphColorBuffer: context.buildGPUBuffer(
        'Glyph Color Buffer',
        new Float32Array(glyphColorBuffer),
        GPUBufferUsage.VERTEX,
      ),
      indexOffset: -1,
      filterLength,
  ...
  #buildFeatures(source: GlyphSource, tile: Tile, featureGuideArray: Float32Array): void {
    ...
    while (i < lgl) {
      ...
      const glyphBoundsBuffer = context.buildGPUBuffer(
        'Glyph Bounds Buffer',
        new Float32Array([0, 0, 1, 1]),
        GPUBufferUsage.UNIFORM,
      );
      const glyphUniformBuffer = context.buildGPUBuffer(
        'Glyph Uniform Buffer',
        new Float32Array([0, isPath, isIcon, ~~overdraw, 1, 1]),
        GPUBufferUsage.UNIFORM,
      );
      const glyphAttributeBuffer = context.buildGPUBuffer(
        'Glyph Attributes with Stroke Buffer',
        new Uint32Array([filterOffset, filterCount, 1]),
        GPUBufferUsage.UNIFORM,
      );
      const glyphAttributeNoStrokeBuffer = context.buildGPUBuffer(
        'Glyph Attributes Buffer',
        new Uint32Array([filterOffset, filterCount, 0]),
        GPUBufferUsage.UNIFORM,
      );
