...
uniform float uRadiusLo;
uniform float uOpacityLo;
uniform float uIntensityHi;
uniform float uRadiusHi;
uniform float uOpacityHi;
...
void main () {
  ...
  if (uDrawState == 0.) { // drawing to texture
    ...
    float intensity = uIntensityLo + t * (uIntensityHi - uIntensityLo);
    float radius = (uRadiusLo + t * (uRadiusHi - uRadiusLo)) * uDevicePixelRatio * 2.;
    vOpacity = uOpacityLo + t * (uOpacityHi - uOpacityLo);
