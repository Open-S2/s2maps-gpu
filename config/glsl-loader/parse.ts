...
function parse(
  id: string,
  contents: string,
  res: Result = { source: '', uniforms: {}, attributes: {}, variables: { main: 'main' } },
) {
  const path = id.split('/');
  path.pop();
  const pathCleaned = path.join('/');

  let shaderName = id.split('/').pop() as string;
  ...
  _parse(contents, pathCleaned, res, shaderName);
 ...
 * @param shaderName
 */
function _parse(contents: string, path: string, res: Result, shaderName: string) {
  // 1) break into lines
  const contentList = contents.split('\n');

  for (const line of contentList) parseLine(line, path, res, shaderName);
}
 ...
 * @param shaderName
 */
function parseLine(line: string, path: string, res: Result, shaderName: string): void {
 ...
 * @param shaderName
 */
function _importCode(line: string, path: string, res: Result, shaderName: string): void {
 ...
 * @param res
 */
function _define(line: string, res: Result): void {
 ...
 * @param shaderName
 */
function _parseLine(line: string, res: Result, shaderName: string): void {
 ...
 * @param shaderName
 */
function _parseWords(words: string[], res: Result, shaderName: string): void {
 ...
 * @param tokenCount
 */
function getMinifiedName(tokenCount: number): string {
  const res = [];
  while (tokenCount > 51) {
    res.push('Z');
    tokenCount -= 51;
...
/**
 * @param source
 * @param source.charPos
 */
function newChar(source: { charPos: number }): string {
