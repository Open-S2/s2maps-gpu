...
layout (location = 0) in float aStep;
...
void main () {
  ...
  if (uMode == 0) {
    ...
    if (aStep == 0.) {
      ...
      // set position
      gl_Position = vec4(2. * ((aIndex + uIndexOffset) / 4096.) - 1., 0., 0., 1.);
    } else {
      ivec2 res = ivec2(ceil(bottomLeft + WH));
      color = vec4(float(res.x >> 8) / 255., float(res.x & 255) / 255., float(res.y >> 8) / 255., float(res.y & 255) / 255.);
      // set position
      gl_Position = vec4(2. * ((2048. + aIndex + uIndexOffset) / 4096.) - 1., 0., 0., 1.);
    }
    ...
    if (colorID == ivec3(point.rgb * 256.)) {
      color = vec4(point.rgb, 1.);
      // assuming the depth test has passed, we loop through the quads to see if their is overlap
