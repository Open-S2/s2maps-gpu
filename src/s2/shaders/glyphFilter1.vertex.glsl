...
vec4 getBbox (float bboxIndex) {
  vec4 btmLeft = texture2D(uQuads, vec2(bboxIndex / 2048., 0.));
  vec4 topRght = texture2D(uQuads, vec2(bboxIndex / 2048., 1.));
  return vec4(
...
void main () {
  ...
  if (uMode == 1) {
    gl_PointSize = 1.;
    // set position
    if (aStep == 0.) gl_Position = vec4(2. * ((0.5 + aIndex + uIndexOffset) / 2048.) - 1., -0.5, 0., 1.);
    else gl_Position = vec4(2. * ((0.5 + aIndex + uIndexOffset) / 2048.) - 1., 0.5, 0., 1.);
    ...
    // set color to id
    int id = int(aID);
    vColor = vec4(float(AND(id, 255)) / 255., float(AND(rightShift(id, 8.), 255)) / 255., float(rightShift(id, 16.)) / 255., 1.);
