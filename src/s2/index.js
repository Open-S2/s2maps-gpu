...
export class S2Map {
  ...
  _setupCanvas (canvas: HTMLCanvasElement, options: MapOptions) {
    ...
    if (canvas.transferControlToOffscreen) {
      ...
      this.map.postMessage({ type: 'canvas', options, canvas: offscreen }, [offscreen])
    } else { this.map = new Map(options, canvas) }
  }
  ...
  _resize () {
    ...
    const width = this._container.clientWidth * this._canvasMultiplier | 0
    const height = this._container.clientHeight * this._canvasMultiplier | 0
    if (this._offscreen) {
      this.map.postMessage({ type: 'resize', width, height })
    } else {
