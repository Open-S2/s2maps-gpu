...
export default class Animator {
  ...
  constructor (projector: Projector, directions?: AnimationDirections = {}) {
    ...
    this._setupVariables(directions)
  }

  _setupVariables (directions: AnimationDirections) {
    const { projector } = this
    // setup animation paramaters
    if (directions.easing) this._setEasing(directions.easing)
  ...
  _setEasing (easing: Easing) {
    if (easing === 'linear') this.ease = easeLinear
    else if (easing === 'ease-in') this.ease = easeInExpo
    else if (easing === 'ease-out') this.ease = easeOutExpo
    else if (easing === 'ease-in-out') this.ease = easeInOutExpo
  }
  ...
  increment (time: number): boolean {
    ...
    if (time - this.startTime >= duration) return true
    return false
  }
...
function easeLinear (time: number, start: number, delta: number, duration: number) {
