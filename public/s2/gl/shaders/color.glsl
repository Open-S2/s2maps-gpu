...
vec4 colorBlindAdjust (in vec4 rgba, in float colorMode) {
  // setup rgb
  float r = rgba.r * 255.;
  float g = rgba.g * 255.;
  float b = rgba.b * 255.;
  ...
  float CVD[9];
  if (colorMode == 1.) CVD = float[9](0.0, 2.02344, -2.52581, 0., 1., 0., 0., 0., 1.); // protanopia
  else if (colorMode == 2.) CVD = float[9](1.0, 0., 0., 0.494207, 0., 1.24827, 0., 0., 1.); // deutranopia
