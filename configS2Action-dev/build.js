...
const dirPath = path.join(__dirname, '../buildS2Action-dev')
removeDir(dirPath)
...
async function storeAll () {
  ...
  if (fs.existsSync('../../web/s2maps.io/public/s2maps-gl')) {
    ...
    try { fs.mkdirSync(`../../web/s2maps.io/public/s2maps-gl/${VERSION}`) } catch (_) {}
    const files = fs.readdirSync('./buildS2Action-dev')
    for (const file of files) {
      if (file.endsWith('.min.css') || file.endsWith('.min.js')) {
        fs.copyFileSync(`./buildS2Action-dev/${file}`, `../../web/s2maps.io/public/s2maps-gl/${VERSION}/${file}`)
      }
    }
